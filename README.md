...
its closeness to the goal, and how clear the shot is.
...
1. **Features** (`lesson1_features.html`) — Reality is a function of inputs.
   Explore in 3D how a shot's make-probability changes with its arc, closeness to
   the goal, and clearness.
...
features (x1 = arc, x2 = closeness to goal, x3 = clearness) into a make-probability
between 0 and 100%:

```
make % = 100 · exp((x1 − 50 − 0.3·x2)^2 / −3000) · ln((x2 + 10) / 10) / ln(11) · (x3 / 100)
```
